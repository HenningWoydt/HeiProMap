...
namespace HeiProMap {
    ...
    void compare_uncoarsening(const std::string &graph_in){
        ...
        for (size_t i = 0; i < g_matches.size(); ++i) {
            u64 idx = g_matches.size() - 1 - i;
            vertex_t u = g_matches[idx].u;
            vertex_t v = g_matches[idx].v;

            g.uncontract(u, v);
            g_av_manager.uncontract(u, v);
        }
        ...
        for (size_t i = 0; i < csr_g_matches.size(); ++i) {
            u64 idx = csr_g_matches.size() - 1 - i;
            vertex_t u = csr_g_matches[idx].u;
            vertex_t v = csr_g_matches[idx].v;

            csr_g.uncontract(u, v);
            csr_g_av_manager.uncontract(u, v);
        }
        ...
        std::cout << "uncoarsening " << graph_in << " : g = " << t_g << " csr_g = " << t_csr_g << " speedup = " << t_g / t_csr_g << std::endl;

        graphs_are_equal(g, g_av_manager, csr_g, csr_g_av_manager);
        graphs_are_equal(g, csr_g);
    }

    TEST(GraphUncoarsening, graph_csrgraph_PGPgiantcompo_graph) {
        std::string graph_in = "../data/mapping/PGPgiantcompo.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_cop20k_A_mtx_graph) {
        std::string graph_in = "../data/mapping/cop20k_A.mtx.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_2cubes_sphere_mtx_graph) {
        std::string graph_in = "../data/mapping/2cubes_sphere.mtx.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_thermomech_TC_mtx_graph) {
        std::string graph_in = "../data/mapping/thermomech_TC.mtx.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_598a_graph) {
        std::string graph_in = "../data/mapping/598a.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_cfd2_mtx_graph) {
        std::string graph_in = "../data/mapping/cfd2.mtx.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_boneS01_mtx_graph) {
        std::string graph_in = "../data/mapping/boneS01.mtx.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_fe_ocean_graph) {
        std::string graph_in = "../data/mapping/fe_ocean.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_144_graph) {
        std::string graph_in = "../data/mapping/144.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_Dubcova3_mtx_graph) {
        std::string graph_in = "../data/mapping/Dubcova3.mtx.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_bmwcra_1_mtx_graph) {
        std::string graph_in = "../data/mapping/bmwcra_1.mtx.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_G2_circuit_mtx_graph) {
        std::string graph_in = "../data/mapping/G2_circuit.mtx.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_wave_graph) {
        std::string graph_in = "../data/mapping/wave.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_shipsec5_mtx_graph) {
        std::string graph_in = "../data/mapping/shipsec5.mtx.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_cont_300_mtx_graph) {
        std::string graph_in = "../data/mapping/cont-300.mtx.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_m14b_graph) {
        std::string graph_in = "../data/mapping/m14b.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_auto_graph) {
        std::string graph_in = "../data/mapping/auto.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_afshell9_graph) {
        std::string graph_in = "../data/mapping/afshell9.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_thermal2_graph) {
        std::string graph_in = "../data/mapping/thermal2.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_nlr_graph) {
        std::string graph_in = "../data/mapping/nlr.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_deu_graph) {
        std::string graph_in = "../data/mapping/deu.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_del23_graph) {
        std::string graph_in = "../data/mapping/del23.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_rgg23_graph) {
        std::string graph_in = "../data/mapping/rgg23.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_del24_graph) {
        std::string graph_in = "../data/mapping/del24.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_rgg24_graph) {
        std::string graph_in = "../data/mapping/rgg24.graph";
        compare_uncoarsening(graph_in);
    }

    TEST(GraphUncoarsening, graph_csrgraph_eur_graph) {
