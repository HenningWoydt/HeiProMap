...
- **Dependencies**: The project depends on **KaHIP**, **TBB**, and **GoogleTest**. The `build.sh` script is responsible for downloading and building these locally in `extern/local/`.
...
- **Build with Tests**:
  ...
  ./build.sh -t
  ```
  *Note: This enables assertions (`ENABLE_ASSERTS=ON`) but maintains the current build type (defaults to Release).*
...
## Testing Procedures
1. **Build the tests**: Ensure you use the `-t` flag with `build.sh`.
2. **Run with gtest-parallel (Recommended for speed)**:
   ```bash
...
- **Header Changes**: Since the library is header-only, changes to `src/*.h` files will affect all executables and tests.
- **GTest Integration**: Always add new tests to `tests/` and ensure they are registered in `tests/CMakeLists.txt` (which uses `gtest_discover_tests`).
