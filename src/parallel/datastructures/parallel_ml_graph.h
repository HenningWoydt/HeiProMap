#ifndef HEIDELBERGPROCESSMAPPING_PARALLEL_ML_GRAPH_H
#define HEIDELBERGPROCESSMAPPING_PARALLEL_ML_GRAPH_H
...
namespace HeiProMap {
    ...
    */
    class ParallelMLGraph final : public IParallelGraph {
    ...
    private:
        ...
        std::mutex m_vec_mem_to_free_lock;
        std::vector<std::vector<EdgeVW>*> m_memory_to_free;
        std::vector<std::vector<std::vector<EdgeVW>*>> m_adj_per_level;

    public:
        ParallelMLGraph() = default;

        ~ParallelMLGraph() final {
#pragma omp parallel for schedule(static) default(none) num_threads(m_n_threads)
            for(size_t i = 0; i < m_memory_to_free.size(); ++i){
                free(m_memory_to_free[i]);
            }
        }
        ...
        void initialize(const std::string &graph_in, u64 n_threads) final {
            ...
            m_adj_per_level.emplace_back(m_n, nullptr);
#pragma omp parallel default(none) firstprivate(i, file_size, file_arr, fmt_1, fmt_2, n_threads) num_threads(n_threads)
            {
                ...
                while (true) {
                    ...
                    m_adj_per_level[0][u] = new std::vector<EdgeVW>(edges);
                    std::sort(m_adj_per_level[0][u]->begin(), m_adj_per_level[0][u]->end());
                    edges.clear();
...
#pragma omp atomic
        ...
        size_t size(vertex_t u) const final { return m_adj_per_level.back()[u]->size(); }

        vertex_t neighbor(vertex_t u, size_t idx) const final { return  (*m_adj_per_level.back()[u])[idx].v; }
        weight_t get_weight(vertex_t u, size_t idx) const final { return  (*m_adj_per_level.back()[u])[idx].w; }

        // edge manipulation
        bool edge_exists(vertex_t u, vertex_t v) const final { return std::any_of(m_adj_per_level.back()[u]->begin(), m_adj_per_level.back()[u]->end(), [&](const EdgeVW &e) { return e.v == v; }); }
...
#endif //HEIDELBERGPROCESSMAPPING_PARALLEL_ML_GRAPH_H
