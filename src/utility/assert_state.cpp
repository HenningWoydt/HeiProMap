...
namespace HeiProMap {
    ...
    bool assert_n_active_vertices(IGraph &g,
    ...
    bool assert_correct_vertices_active(IGraph &g,
                                        IActiveVertexManager &av_manager) {
    ...
    bool assert_active_edges(IGraph &g,
                             IActiveVertexManager &av_manager) {
        for (av_manager.reset_iterator(); av_manager.available(); av_manager.next()) {
    ...
    bool assert_no_self_loops(IGraph &g,
                              IActiveVertexManager &av_manager) {
        for (av_manager.reset_iterator(); av_manager.available(); av_manager.next()) {
            vertex_t u = av_manager.get();

            for (size_t i = 0; i < g.n_neighbors(u); ++i) {
                for (size_t j = i + 1; j < g.n_neighbors(u); ++j) {
                    const EdgeW &e1 = g.neighbor(u, i);
                    const EdgeW &e2 = g.neighbor(u, j);
                    ASSERT(e1.v != e2.v);
                }
            }
        }
    ...
    bool assert_n_boundary_vertices(IGraph &g,
                                    IActiveVertexManager &av_manager,
                                    IPartitionManager &p_manager,
    ...
    bool assert_correct_vertices_boundary(IGraph &g,
                                          IActiveVertexManager &av_manager,
                                          IPartitionManager &p_manager,
                                          IBoundaryVertexManager &bv_manager) {
        std::vector<vertex_t> manual;
    ...
    bool assert_correct_vertices_boundary_per_block(IGraph &g,
                                                    IActiveVertexManager &av_manager,
                                                    IPartitionManager &p_manager,
                                                    IBoundaryVertexManager &bv_manager,
                                                    partition_t k) {
