#ifndef SERIALPROCESSMAPPING_BOUNDARY_VERTEX_ITERATOR_H
#define SERIALPROCESSMAPPING_BOUNDARY_VERTEX_ITERATOR_H
...
namespace SPM {
    ...
    public:
        explicit BoundaryVertexIterator(BoundaryVertexManager &bvm) : partition(bvm.get_partition()), n_boundary_edges(bvm.get_n_boundary_edges()), boundaries(bvm.get_boundaries()) {
            k = bvm.get_k();
            b_idx = 0;
            indices.resize(k, 0);
        }
...
#endif //SERIALPROCESSMAPPING_BOUNDARY_VERTEX_ITERATOR_H
